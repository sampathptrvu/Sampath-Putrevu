...
let newContent = `<h3 className="font-serif font-semibold text-[22px] md:text-[24px] lg:text-[28px] text-[var(--color-text)] mb-[24px]">
                  At Accel's SeedToScale
                </h3>`;

data.forEach((section, index) => {
  newContent += `\n                <h4 className="font-sans font-medium text-[16px] lg:text-[18px] text-[var(--color-text)] mb-[16px] ${index > 0 ? 'mt-[32px]' : ''}">${section.category}</h4>`;
...
const regex = /(<h3 className="font-serif font-semibold text-\[22px\] md:text-\[24px\] lg:text-\[28px\] text-\[var\(--color-text\)\] mb-\[24px\]">\s*At Accel's SeedToScale\s*<\/h3>\s*<ul className="flex flex-col gap-\[16px\]">.*?<\/ul>)/s;

if (regex.test(fileContent)) {
  fileContent = fileContent.replace(regex, newContent);
  fs.writeFileSync('src/PublishedPage.tsx', fileContent);
  console.log("Replaced At Accel's SeedToScale successfully.");
} else {
