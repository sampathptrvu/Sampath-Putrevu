...
const replacements = [
  {
    replace: `<span className="font-sans font-normal text-[15px] lg:text-[16px] leading-[1.55] text-[var(--color-text)]">\n                      Mapping the journey of Lalitesh Katragadda, the creator of Google Maps India\n                    </span>`,
...
for (const rep of replacements) {
  if (content.includes(rep.find)) {
    ...
    // If exact match failed, try a looser regex replacement
    const urlMatch = rep.find.match(/href="([^"]+)"/);
    if (urlMatch) {
      const url = urlMatch[1];
      // escape regex
      const escapedUrl = url.replace(/[.*+?^$\{}()|[\]\\]/g, '\\$&');
      const regex = new RegExp(`<a[^>]*href="${escapedUrl}"[^>]*>\\s*(.*?)\\s*</a>`, 's');
      if (regex.test(content)) {
        content = content.replace(regex, `<span className="font-sans font-normal text-[15px] lg:text-[16px] leading-[1.55] text-[var(--color-text)]">\n                      $1\n                    </span>`);
        replacedCount++;
      }
    }
  }
...
console.log(`Reverted ${replacedCount} links in PublishedPage.tsx`);
